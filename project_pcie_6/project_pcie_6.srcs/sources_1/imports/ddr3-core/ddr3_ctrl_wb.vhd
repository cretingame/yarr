...
architecture behavioral of ddr3_ctrl_wb is
    ...
    signal wb_rd_read_req_nb_s : unsigned(1 downto 0);
...
begin
                ...
                if(wb_write_cnt >= 3) then
                 ...
                 if(wb_write_cnt /= 0) then
                     ...
                     if (wb_write_wait_cnt = 0) then
                         ...
                         elsif (wb_write_cnt = 2) then
                             fifo_wb_wr_addr_din_s <= std_logic_vector(UNSIGNED(wb_wr_addr_s) + 2);
                             fifo_wb_wr_mask_din_a(3) <= X"00";
                             fifo_wb_wr_mask_din_a(2) <= X"00";
                         elsif (wb_write_cnt = 1) then
                             fifo_wb_wr_addr_din_s <= std_logic_vector(UNSIGNED(wb_wr_addr_s) + 3);
                             fifo_wb_wr_mask_din_a(3) <= X"00";
                             fifo_wb_wr_mask_din_a(2) <= X"00";
                             fifo_wb_wr_mask_din_a(1) <= X"00";
                         elsif (wb_write_cnt = 0) then
                             fifo_wb_wr_addr_din_s <= std_logic_vector(UNSIGNED(wb_wr_addr_s) + 4);
                             fifo_wb_wr_mask_din_a(3) <= X"00";
                             fifo_wb_wr_mask_din_a(2) <= X"00";
                             fifo_wb_wr_mask_din_a(1) <= X"00";
                         end if;
    ...
    fifo_wb_wr_din_s <= fifo_wb_wr_addr_din_s &
                        ...
                        fifo_wb_wr_mask_din_a(2) & 
                        fifo_wb_wr_mask_din_a(1) & 
                        fifo_wb_wr_mask_din_a(0) &
                        fifo_wb_wr_data_din_a(3) & 
                        fifo_wb_wr_data_din_a(2) & 
                        fifo_wb_wr_data_din_a(1) & 
                        fifo_wb_wr_data_din_a(0); 
    ...
    if (rst_n_i = '0') then
        ...
        if (wb_cyc_i = '1' and wb_stb_i = '1' and wb_we_i = '0') then
            ...
            if (wb_read_cnt >= 3) then
                wb_read_test <= 2;
                wb_read_cnt <= 0;
                wb_rd_read_req_nb_s <= TO_UNSIGNED(3,2);
                fifo_wb_rd_addr_wr_s <= '1';
            else
                wb_read_test <= 3;
                wb_read_cnt <= wb_read_cnt + 1; -- ICI
                wb_read_wait_cnt <= c_read_wait_time;
                fifo_wb_rd_addr_wr_s <= '0';
            end if;
        else
             ...
             if wb_read_cnt /= 0 then
                ...
                if wb_read_wait_cnt = 0 then
                    wb_read_test <= 6;
                    wb_rd_read_req_nb_s <= TO_UNSIGNED(wb_read_cnt,2);
                    wb_read_cnt <= 0;
                    fifo_wb_rd_addr_wr_s <= '1';       
                end if;   
             end if;
    ...
    begin
        if (rst_n_i = '0') then
            ...
            if wb_read_data_cnt /= 3 then
                wb_read_data_cnt <= wb_read_data_cnt + 1;
            end if;
        end if;
    ...
    fifo_wb_rd_addr_din_s <= fifo_wb_rd_addr_din_a(wb_read_cnt);
    
    fifo_wb_rd_data_rd_s <= not fifo_wb_rd_data_empty_s;
    
    wb_rd_ack_s <= '1' when wb_read_data_cnt /= 3 or wb_read_data_cnt_d /=3 else
                   '0'; 
    ...
    -- Stall proc
    --------------------------------------
	wb_stall_s <= fifo_wb_wr_full_s or (not ddr_wdf_rdy_i) or (not ddr_rdy_i);
